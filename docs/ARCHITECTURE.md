...
│                               │                             │
│  ┌──────────┐    ┌────────────┼────────────┐               │
│  │ Claude   │───▶│ 프로세스 B: MCP 서버     │               │
│  │ Desktop  │◀───│ python -m domae_mcp     │               │
│  └──────────┘    │   --mcp (stdio)         │               │
│                  │ - 12개 Tools              │               │
│                  └────────────┬────────────┘               │
│                               │                             │
...
> **SQLite 동시 접근**: 두 프로세스가 같은 DB를 사용하므로 WAL 모드 활성화 + `busy_timeout=5000`ms 설정 필수.
> `engine = create_engine(url, connect_args={"check_same_thread": False, "timeout": 5})`
> 그리고 연결 시 `PRAGMA journal_mode=WAL;` 실행.
...
```
~/.maipharm-domae-mcp/
├── config.json        # 도매 계정 (암호화), 텔레그램, 설정
├── .key               # Fernet 암호화 키 (자동 생성, git 무시)
└── data/
    └── domae.db       # SQLite 데이터베이스 (WAL 모드)
```
...
- 암호화 키는 `~/.maipharm-domae-mcp/.key` 파일에 저장 (최초 실행 시 자동 생성)
- `.key` 파일이 없으면 새로 생성, 있으면 기존 키 사용
...
| 신규도매 메일 | SendGrid API 키는 패키지에 내장 (발송 전용, 수신 불가), HTTPS 전송 |
